...
import PickDeliveryAddressSection from "../../../components/orden/PickDeliveryAddress";
...
import SetDeliveryCostSection from "../../../components/orden/SetDeliveryCost";

// ----------------------------------------------------------------------

// ----------------------------------------------------------------------

type OrdenDeliveryViewProps = {
  clientId?: string;
  editMode?: boolean;
  viewMode?: "orden" | "reInvoice";
};

const OrdenDeliveryView: React.FC<OrdenDeliveryViewProps> = ({
  clientId,
  editMode = false,
  viewMode = "orden",
}) => {
  ...
  const [quant, _setQuant] = useState<string>(newOrden.shippingCost);
          ...
          try {
            await dispatch(
              submitReInvoiceOrden(
                ...
                newOrden.shippingCost || 0
              )
            );
            enqueueSnackbar("Pedido refacturado correctamente", {
              variant: "success",
            });
            mixtrack("reInvoiceOrden", {
            ...
            if (error.message === "Error: La factura tiene complemento") {
              enqueueSnackbar("Error al re facturar la factura tiene complementos de pago", {
                variant: "error",
              });
            }
            else {
              enqueueSnackbar("Error al re facturar la orden", {
                variant: "error",
              });
            }
          ...
          await dispatch(
            submitNewOrdenNormal(
              business.id,
              {
                ...
                shippingCost: newOrden.shippingCost || 0,
              },
              sessionToken || ""
            )
          );
  ...
  const updateDeliveryCost = (deliveryDetail: string, value: any) => {
    if (typeof value === "string") {
      _setQuant(value);
      dispatch(updateNewOrden({ key: deliveryDetail, value: value }));
      return;
      ...
      if (value < 0) {
        _setQuant("0");
        dispatch(updateNewOrden({ key: deliveryDetail, value: 0 }));
        return;
      }
      if (value === 0) {
        _setQuant("");
      ...
      _setQuant(_val.toString());
      dispatch(updateNewOrden({ key: deliveryDetail, value: _val }));
    }
  };
  ...
  return (
    <Box sx={{ mt: theme.spacing(2) }}>
      <BasicDialog
        open={confirmModalOpen || ordenSubmitted?.id !== undefined}
      ...
      {newOrdenCurrentStep === 1 && (
        <Box>
          ...
          <PickDeliveryAddressSection
            address={newOrden.restaurantBranch?.fullAddress || ""}
      ...
      {newOrdenCurrentStep === 2 && (
        <Box>
          ...
          />
          {/* Step 3: Add delivery cost */}
          <SetDeliveryCostSection
            deliveryCost={quant}
            updateDeliveryCost={(k: string, v: any) => {
              updateDeliveryCost(k, Number(v));
...
export default OrdenDeliveryView;
