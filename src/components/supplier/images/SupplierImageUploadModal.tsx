...
import { addSupplierBusinessImage } from "../../../redux/slices/account";

// ----------------------------------------------------------------------

type SupplierImageUploadModalProps = {
  open: boolean;
...
const SupplierImageUploadModal: React.FC<SupplierImageUploadModalProps> = ({
  open,
  onClose,
  supplierBusinessId,
  editMode = false,
  ...
  const handleUploadImage = async () => {
    setLoading(true);
    mixtrack("supplier_business_image_upload", {
      visit: window.location.toString(),
      page: "ProductDetails",
      section: "SupplierProductImageSelector",
      supplierBusinessId,
    });
    try {
      if (!sessionToken) {
      ...
      // Add image
      await dispatch(
        addSupplierBusinessImage(sessionToken, files[0], supplierBusinessId)
      );
      ...
      <DialogContent>
        <Grid container sx={{ mt: { xs: 3, md: 3 } }}>
          ...
          <Grid item xs={12} md={12}>
            ...
            >
              {`Sube la imagen de tu producto en formato (.png, .jpg, .jpeg, .webp o .HEIC) para ${
...
export default SupplierImageUploadModal;
